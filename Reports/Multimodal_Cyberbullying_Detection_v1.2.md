...
### Full Project Scope & Technical Analysis  *(v1.2 — architecture enhanced after literature review)*
...
### 8.2 Branch A — CLIP Image Encoder Module *(upgraded in v1.2)*

**Goal:** Extract visual cues for hate speech using a text-aligned image encoder pretrained
on 400M image-text pairs.

- **Model:** CLIP ViT-B/16 image encoder (OpenAI), pretrained via contrastive image-text learning
- **Why CLIP over ImageNet-21K ViT (v1.2 upgrade):** CLIP was trained on 400M image-text pairs
  using contrastive learning, so its image features are *already aligned with text semantics* —
  exactly what multimodal hate speech detection needs. ImageNet-21K is trained on natural images
  with single-class labels (cats, dogs, etc.), which is irrelevant for memes, screenshots, and
  meme-style hate content. Recent MMHS150K literature (Shah et al. 2024 MemeCLIP, Burbi et al.
  2023, Arya et al. 2024, Kumar & Nandakumar 2022 Hate-CLIPper) all use CLIP for this reason.
- **Input:** 3-channel RGB image, 224×224, CLIP normalisation (different from ImageNet — uses
  CLIP-specific mean and std)
- **Fine-tuning:** **LoRA adapters** (rank 16) on attention layers, base weights frozen. This
  is a v1.2 upgrade — full fine-tuning is replaced with parameter-efficient LoRA.
- **Why LoRA:** (1) Fits 6GB GPU constraint by reducing trainable parameters by ~99%;
  (2) Prevents catastrophic forgetting of CLIP's pretrained image-text alignment;
  (3) 2024 standard for parameter-efficient transfer learning;
  (4) No 2020–2023 MMHS150K paper uses LoRA — modest novelty contribution.
...
- **Output:** 512-d [CLS]-equivalent embedding from CLIP image encoder
  (note: CLIP ViT-B/16 outputs 512-d, not 768-d like ImageNet ViT)
- **Auxiliary task during fine-tuning:** Predict T1 (binary hate) directly from CLIP head before
  fusion training to give the backbone domain awareness

---

### 8.3 Branch B — Twitter-RoBERTa Text Module *(upgraded in v1.2)*

**Goal:** Decode hate language including slang, slurs, sarcasm, and coded references — with
a model already familiar with Twitter language.

- **Model:** `cardiffnlp/twitter-roberta-base-2022-154m` — RoBERTa-base pretrained on 154M
  Twitter posts through 2022
- **Why Twitter-RoBERTa over RoBERTa-base (v1.2 upgrade):** RoBERTa-base is pretrained on
  BookCorpus and Wikipedia — formal written English. Twitter content uses different vocabulary,
  emojis, hashtags, abbreviations, and grammar conventions. Twitter-RoBERTa was specifically
  pretrained on Twitter data and benchmarks higher than RoBERTa-base on Twitter-specific tasks.
  The CASE 2024 winning entry (Wang & Markov 2024) used this exact model family.
- **Fine-tuning strategy:**
  1. **Stage 1 — Cyberbullying Kaggle pre-training:** Fine-tune Twitter-RoBERTa on Cyberbullying
     Kaggle dataset (47K tweets, 4-class) for 3 epochs. Gives hate-speech-specific awareness.
  2. **Stage 2 — MMHS150K fine-tuning:** Fine-tune on MMHS150K with auxiliary T1 prediction
     head for 5 epochs.
  3. **LoRA adapters** (rank 16) on attention layers — base weights frozen. v1.2 upgrade.
- **Why LoRA on text branch:** Same reasoning as image branch — fits memory constraints,
  prevents catastrophic forgetting of Twitter-specific pretraining, modern technique.
- **Tokenisation:** Twitter-RoBERTa tokeniser (BPE with Twitter-specific extensions for
  emojis, hashtags). Max sequence length 128 tokens (re-verify after BPE per EDA Q2).
- **Special handling:** Preserve hashtags, mentions, URLs as tokens. The Twitter-RoBERTa
  tokeniser is already designed for this.
- **Output:** 768-d [CLS] token + token-level hidden states
...
**Mechanism (v1.2 — updated dimensions for CLIP):**

```
# Branch outputs
A_emb  = CLIP image embedding           (512-d, v1.2 changed from 768)
B_cls  = Twitter-RoBERTa [CLS] embedding (768-d)
B_tok  = Twitter-RoBERTa token embeddings (128 × 768)
C_emb  = Structured features (raw)       (9-d after correlation pruning)

# Step 0: Project image embedding to match text dimensionality
A_proj = Linear(512 → 768)              (v1.2 — needed for cross-attention compatibility)

# Step 1: Compute per-modality gate weights
gate_input = concat([A_proj, B_cls, C_emb])   → (1545-d)
gate_logits = FC(1545 → 3)                     → (3,)
g_A, g_B, g_C = softmax(gate_logits)            three weights summing to 1

# Step 2: Modality-weighted cross-attention (image attends to text)
Q = Linear(A_proj.unsqueeze(1))                → (1, 512)
K = Linear(B_tok)                              → (128, 512)
V = Linear(B_tok)                              → (128, 512)

cross_attn_AB = softmax(Q·K^T / √512) · V      → (1, 512)
visual_text   = cross_attn_AB.squeeze(0) · g_A  gated by image relevance

# Step 3: Final gated fused representation
fused = concat([
    g_A · A_proj,           # image contribution, gated
    g_B · B_cls,             # text contribution, gated
    g_C · C_emb,             # structured contribution, gated
    visual_text              # cross-modal interaction
])                                              → (2057-d)

shared_repr = FC(2057 → 512) → ReLU → BN → Dropout(0.3)
```
...
### 8.5c — Future Stretch Goal: Mixture-of-Experts Fusion *(deferred, not committed)*

**Status:** Documented for future exploration. **NOT part of v1.2 implementation plan.**

If MVP 4 (gated cross-modal attention) is fully validated and time permits before MVP 5, an
MoE-style fusion may be explored as a comparison architecture with three "expert" sub-networks
(text-dominant, image-dominant, multimodal-required) routed by a learned gating network.

**Conditions for MoE exploration (all must be met):**
- Gated cross-modal attention must be validated first
- MoE explored only as a *secondary* experiment with timebox of 3 weeks
- Reverted if not outperforming gated fusion baseline
- Switch Transformer load-balancing recipe (Fedus et al. 2022)

**Risks acknowledged:** 6GB GPU memory pressure, load-balancing tuning complexity, multiple
failure modes. This is documented as a *research direction*, not a deliverable.

### 8.5d — The Honest Hypothesis
...
### 8.7 Training Enhancements *(new in v1.2)*

Beyond the architectural upgrades, v1.2 adds modern training practices absent from older
MMHS150K work:

**Two-stage learning rate schedule:**
- Linear warmup over first 10% of training steps
- Cosine decay from peak to 1% of peak over remaining 90%
- Different peak learning rates per component:
  - Twitter-RoBERTa LoRA adapters: 1e-4
  - CLIP LoRA adapters: 1e-4
  - Gate + fusion layers: 1e-3
  - Output heads: 1e-3

**Gradient accumulation:**
- Physical batch size: 16 (hardware constraint on 6GB GPU)
- Accumulation steps: 4
- Effective batch size: 64
- Matches 2024 standard for stable transformer training

**Stratified sampling within hate categories:**
- `WeightedRandomSampler` with inverse frequency weights at T2 level
- Each hate category receives equal training exposure
- Prevents the model from over-fitting to Racist class (most common hate type)

**Class-balanced weighted loss:**
- Class weights computed from training distribution only (~22% hate, ~78% no-hate)
- Applied per-sample in Focal Loss
- Do NOT use val/test distribution (50/50) for weighting — that would leak split info

### 8.8 Per-Sample Modality Reliance Analysis *(new in v1.2 — differentiation contribution)*

**Goal:** Build the empirical analysis component that distinguishes this work from existing
MMHS150K papers. No paper in the 20-paper literature review performs systematic per-sample
analysis of modality interactions.

**Method:**

For every test sample, record:
- Text-only model's prediction and confidence
- Image-only model's prediction and confidence
- Gated fusion's prediction, confidence, and learned gate weights (g_A, g_B, g_C)
- Ground truth T1 label

Then categorize each sample into one of four interaction patterns:

| Pattern | Definition | What it tells us |
|---------|-----------|------------------|
| **Convergent correct** | Both unimodal correct, fusion correct | Easy cases; fusion shouldn't hurt |
| **Text saved** | Text correct, image wrong, fusion correct | Text branch dominates |
| **Image saved** | Image correct, text wrong, fusion correct | Image branch contributed |
| **Emergent multimodal** | Both unimodal wrong, fusion correct | Genuine multimodal reasoning required |
| **Fusion failure** | Both unimodal correct, fusion wrong | Fusion confused; needs investigation |

**Report deliverables:**
1. **Distribution of interaction patterns** across the full test set
2. **Per-pattern gate weight statistics** — does the gate route correctly?
3. **Confidence calibration per pattern** — are easy cases more confident than hard cases?
4. **Failure mode browser** — searchable display of 20+ "emergent multimodal" examples and
   20+ "fusion failure" examples in the Streamlit dashboard

**Why this differentiates the project:** Existing MMHS150K papers report aggregate metrics
(macro F1, AUC). This analysis identifies *when and why* multimodal reasoning helps versus
hurts, providing empirical depth that the broader literature lacks.

---

## 9. AI & ML Concepts Used *(v1.2 updated)*

| Concept | Where Applied |
|---------|---------------|
| **Computer Vision** | CLIP ViT-B/16 image encoder |
| **Vision-Language Pre-training** | CLIP contrastive image-text alignment |
| **Transfer Learning** | CLIP → fine-tune on MMHS150K via LoRA |
| **Parameter-Efficient Fine-Tuning (LoRA)** | Both CLIP and Twitter-RoBERTa |
| **NLP — Transformers** | Twitter-RoBERTa (domain-pretrained) |
| **Two-stage Fine-Tuning** | Cyberbullying Kaggle → MMHS150K |
| **Classical ML** | XGBoost on structured features baseline |
| **Gated Cross-Modal Attention** | Core architectural contribution |
| **Entropy Regularization** | Prevents gate collapse |
| **Multi-Task Learning** | 3 simultaneous prediction heads (T1, T2, T3) |
| **Focal Loss** | Class imbalance handling for T1 and T2 |
| **Masked Loss** | T2 training on non-NaN samples only |
| **Two-stage LR Schedule** | Linear warmup + cosine decay |
| **Gradient Accumulation** | Effective batch 64 on 6GB GPU |
| **Stratified Sampling** | Within hate categories |
| **OCR Feature Engineering** | Image-text region extraction |
| **Sentiment Lexicons** | VADER scores as structured features |
| **Cross-Domain Evaluation** | MMHS150K → Hateful Memes stress test |
| **Per-Sample Modality Analysis** | Differentiation contribution |
| **Bias Mitigation** | Identity masking + counterfactual + subgroup reporting |
| **Explainable AI (XAI)** | GRAD-CAM, BERT attention, gate weights, SHAP |
...
## 11. Technology Stack *(v1.2 updated)*

| Layer | Tool |
|-------|------|
| **Language** | Python 3.11 |
| **Deep Learning** | PyTorch 2.x |
| **Computer Vision** | CLIP (openai/clip-vit-base-patch16 via HuggingFace) |
| **NLP** | HuggingFace Transformers (cardiffnlp/twitter-roberta-base-2022-154m) |
| **Parameter-Efficient FT** | PEFT (LoRA adapters) |
| **Tabular ML** | XGBoost (CPU baseline only) |
...
## 18. Academic Contribution *(v1.2 — strengthened with literature-informed positioning)*

**Refined contribution framing:** This project does not invent gated multimodal fusion or any
individual architectural component. The contribution is *the combination of modern multimodal
techniques applied to the documented MMHS150K failure mode, with diagnostic rigor and
empirical analysis absent from comparable work.* This wording is academically defensible.

### Four-Pronged Contribution

1. **Modern Multimodal Architecture for MMHS150K:** This is the first MMHS150K project to
   combine CLIP image features, Twitter-domain-pretrained RoBERTa, LoRA-based parameter-efficient
   fine-tuning, and gated cross-modal attention with entropy regularization. Each individual
   component exists in literature; their combination targeting the documented MMHS150K failure
   mode is novel.

2. **Gate Collapse Diagnostics and Entropy Regularization:** A common failure mode of gated
   multimodal architectures is gate collapse to a single modality. This project explicitly
   monitors gate entropy, regularizes for diversity, and reports per-class gate distributions.
   The 20-paper literature review showed no MMHS150K paper has documented this diagnostic
   methodology.

3. **Per-Sample Modality Reliance Analysis:** Existing MMHS150K papers report aggregate
   metrics. This project provides systematic per-sample analysis categorizing samples by
   modality interaction pattern (Text Saved, Image Saved, Emergent Multimodal, Fusion
   Failure), with failure mode browser deliverable. This empirical depth is absent from
   the broader MMHS150K literature.

4. **Practical Bias Analysis in Hate Speech Detection:** Identity-term masking augmentation,
   counterfactual swap testing, and subgroup performance reporting applied to a known biased
   dataset. Documented mitigation methodology, not fairness solution.
...
- Does not claim cross-language or cross-platform generalization

These honest exclusions strengthen rather than weaken the project — they show academic
maturity. Each claim above can be supported with empirical evidence from the project's
own results.
...
*Document version: 1.2 | Architecture enhanced after 20-paper literature review | Prepared for Final Year CS Project (AI Major)*

## Changelog v1.1 → v1.2 — Architecture Enhancement After Literature Review

### Architectural Upgrades
- **Replaced ImageNet-21K ViT with CLIP ViT-B/16 image encoder** — text-aligned features
  pretrained on 400M image-text pairs (8.2). Used by 4 papers in literature review.
- **Replaced RoBERTa-base with Twitter-RoBERTa** (`cardiffnlp/twitter-roberta-base-2022-154m`)
  — domain-specific pretraining (8.3). Used by CASE 2024 winning entry.
- **Added LoRA adapters** for parameter-efficient fine-tuning on both encoders (8.2, 8.3).
  Modern 2024 technique absent from 2020-2023 MMHS150K papers.

### Training Enhancements (new section 8.7)
- Two-stage learning rate (linear warmup + cosine decay)
- Different learning rates per component (1e-4 LoRA, 1e-3 fusion/heads)
- Gradient accumulation for effective batch 64 on 6GB GPU
- Stratified sampling within hate categories
- Class-balanced weighted loss using train distribution

### New Empirical Contribution (new section 8.8)
- **Per-sample modality reliance analysis** — categorize each test sample by interaction
  pattern (Convergent Correct, Text Saved, Image Saved, Emergent Multimodal, Fusion
  Failure). Empirical depth absent from MMHS150K literature.

### Strengthened Academic Positioning (section 18)
- Four-pronged contribution framing aligned with literature gaps identified in 20-paper review
- Explicit non-claims list to prevent overclaiming attacks at defense

### Documented But Not Committed
- MoE-style fusion as future stretch goal only (8.5c) — *not* part of v1.2 implementation plan
- Conditions documented for if/when MoE may be explored

## Changelog v1.0 → v1.1 — Critique-Driven Revisions
- Removed T4 as supervised target (circular dependency)
- Added MVP ladder for staged training survivability
- Added gate collapse diagnostics + entropy regularisation
- Lightened structured branch (removed TabNet)
- Added bias analysis section (identity masking, counterfactual, subgroup)
- Added error analysis section
- Added defense preparation section
- Softened XAI claims to correlational interpretability
